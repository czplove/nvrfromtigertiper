...
char *readline(char *mbuf, int maxlen, FILE *fp)	//-先读出一整行内容,然后再取出有效部分
{
	int len=0,i=0;
	...
	while((*(buf+len) == 0x20 || *(buf+len) == '\t') && len<maxlen-1)	//-去除前面的空格和制表符
		len++;
...
{ 
    ...
    while(1){ 
            ...
            for(i=0;i<linesnum;i++){
                 pthread_join(id[i], NULL); //-使主线程阻塞等待其他线程退出
            } 
